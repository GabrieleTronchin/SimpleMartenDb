...
public static class EventEndpoint
{
    public static void AddEventsEnpoint(this IEndpointRouteBuilder app)
    {
        app.MapPost(
            ...
            async (IDocumentStore store) =>
            {
                ...
                session.Events.StartStream(
                    carId,
                    new UpdateLocationRequest() { Latitute = 0, Longitude = 0 }
                ); // a stream cannot start without an event
        ...
        app.MapPut(
            "UpdateLocation/{carId}",
            async ([FromBody] UpdateLocationRequest request, Guid carId, IDocumentStore store) =>
            {
                using var session = await store.LightweightSerializableSessionAsync();
        ...
        app.MapGet(
                "LiveAggregation/{carId}",
                async (Guid carId, IQuerySession querySession) =>
                {
                    return await querySession.Events.AggregateStreamAsync<CarAggregateEntity>(
                        carId
                    );
                }
            )
        ...
        app.MapPut(
            "CarMaintenance",
            async ([FromBody] CarMaintenanceEvent request, IDocumentStore store) =>
            {
                using var session = await store.LightweightSerializableSessionAsync();
        ...
        app.MapDelete(
            "CarMaintenance",
            async ([FromBody] RemoveCarMaintenance request, IDocumentStore store) =>
            {
                using var session = await store.LightweightSerializableSessionAsync();
                session.Events.Append(request.CarId, request);
                await session.SaveChangesAsync();
