...
public static class CrudEndpoint
{
    ...
    {
        app.MapPost("/Create", async (IDocumentStore store) =>
        {
        ...
        app.MapPut("/Update/{carId}", async ([FromBody] UpdateLocationRequest request, Guid carId, IQuerySession querySession, IDocumentStore store) =>
        {
            var car = await querySession.Query<CarEntity>().Where(x => x.Id == carId).FirstOrDefaultAsync();
